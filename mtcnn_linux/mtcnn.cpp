...
MTCNN::MTCNN(const string &model_path) {
	...
	std::vector<std::string> bin_files = {
 	...
 	/*
	int ret1=Pnet.load_param_bin(param_files[0].data());
	...
	*/
    ...
    int ret1=Pnet.load_param_bin("../assets/det1.param.bin");
    int ret2 =Pnet.load_model("../assets/det1.bin");
	int ret3 =Rnet.load_param_bin("../assets/det2.param.bin");
	int ret4= Rnet.load_model("../assets/det2.bin");
	int ret5=Onet.load_param_bin("../assets/det3.param.bin");
	int ret6=Onet.load_model("../assets/det3.bin");


	std::cout<<"#####"<<ret1<<" "<<ret2<<" "<<ret3<<" "<<ret4<<" "<<ret5<<" "<<ret6<<std::endl; 
}
...
MTCNN::MTCNN(const std::vector<ncnn::Mat> param_files, const std::vector<ncnn::Mat> bin_files){

    int ret1 = Pnet.load_param((const unsigned char *)param_files[0]);
    int ret2 =  Pnet.load_model((const unsigned char *)bin_files[0]);
    int ret3 = Rnet.load_param((const unsigned char *)param_files[1]);
    int ret4= Rnet.load_model((const unsigned char *)bin_files[1]);
    int ret5 = Onet.load_param((const unsigned char *)param_files[2]);
    int ret6 =  Onet.load_model((const unsigned char *)bin_files[2]);
